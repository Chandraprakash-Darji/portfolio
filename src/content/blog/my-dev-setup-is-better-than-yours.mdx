...
image: '/writing/my-dev-setup-is-better-than-yours/hero.webp'
imageBlurhash: ''
...
import { keyboardKey } from '../../lib/utils.ts';
...
here's the centerpiece: i press <kbd>{keyboardKey('Option')}</kbd><kbd>+</kbd><kbd>f</kbd>. fzf pops up. it's scanning multiple project roots — `~/ghq` at depth 3 (because go modules nest deep), `~/Desktop/p` for personal stuff, `~/Desktop/w` for work, `~/.config` for dotfiles. i type two characters. enter. that's it.
...
the session naming is automatic. pick a folder called `stop-reaching-for-kafka`, you get a tmux session called `stop-reaching-for-kafka`. pick it again later, it attaches to the existing session. no thinking. no `tmux ls`, no `tmux attach -t whatever`. just <kbd>{keyboardKey("Option")}</kbd><kbd>+</kbd><kbd>f</kbd>, pick, you're in.
...
prefix is <kbd>{keyboardKey("Control")}</kbd><kbd>+</kbd><kbd>/</kbd>. not <kbd>{keyboardKey("Control")}</kbd><kbd>+</kbd><kbd>b</kbd>. `/` is right next to right-shift. one finger. try it for a week and then tell me <kbd>{keyboardKey("Control")}</kbd><kbd>+</kbd><kbd>b</kbd> is better.

pane navigation is vim: <kbd>h</kbd> left, <kbd>j</kbd> down, <kbd>k</kbd> up, <kbd>l</kbd> right. no awkward <kbd>{keyboardKey("Control")}</kbd><kbd>+</kbd><kbd>b</kbd> then arrow keys.
...
<kbd>{keyboardKey('Control')}</kbd> <kbd>+</kbd>
<kbd>/</kbd> <kbd>|</kbd> splits vertical. <kbd>{keyboardKey('Control')}</kbd>
<kbd>+</kbd>
<kbd>/</kbd> <kbd>_</kbd> splits horizontal. these are the tmux defaults people
remap anyway, so i just did it.
...
the one that gets people: <kbd>{keyboardKey("Control")}</kbd><kbd>+</kbd><kbd>/</kbd> <kbd>z</kbd> doesn't zoom the current pane. it zooms the _other_ pane. so if i'm in nvim and need to see the terminal output fullscreen, i hit one key. hitting it again restores. small thing. massive quality of life.
...
aerospace is the mac window manager i settled on after trying yabai and amethyst. workspaces 1-5 on <kbd>alt</kbd><kbd>+</kbd><kbd>1</kbd> through <kbd>alt</kbd><kbd>+</kbd><kbd>5</kbd>. gaps at 10px. tiles by default. no mouse needed.
...
<kbd>{keyboardKey('Option')}</kbd>
<kbd>+</kbd>
<kbd>f</kbd> to switch projects. nvim is already open. the terminal is already
there. you don't think about it. you just start working.
