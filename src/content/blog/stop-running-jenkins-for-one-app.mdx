...
image: '/writing/stop-running-jenkins-for-one-app/hero.png'
imageBlurhash: ''
...
sst is a framework for deploying full-stack apps on your own AWS account. it sits on top of Pulumi and Terraform, but instead of making you define raw cloud resources, it gives you high-level components — `sst.aws.Vpc`, `sst.aws.Cluster`, `sst.aws.Service` — that handle the boilerplate. you write TypeScript, not YAML. you describe your app, not your cloud.

it supports next.js, astro, remix, nuxt, sveltekit, express, hono — basically every framework. for our use case (containerized node services), we used the cluster + service pattern. if you're deploying a next.js app, there's a dedicated `sst.aws.Nextjs` component that handles the whole thing.
...
terraform needs modules, state files, providers, backends. its powerful. its also a part-time job for a small team. pulumi is similar — more flexible but still expects you to think in cloud resources.
...
during development, `sst dev` runs your infra in AWS while your app code runs locally — real cloud resources linked to localhost. no "works on my machine" because it literally works against real AWS.
...
read more at [sst.dev/docs](https://sst.dev/docs/).
