...
description: "We were paying $1000/month for AWS MSK to process under 4000 messages a day. Replaced it with BullMQ in a week. Bill dropped to $180. Here's why Kafka was the wrong tool the whole time."
image: ""
imageBlurhash: ""
...
we were running outbound campaigns for sales teams — emails, sms, voicemail. a few thousand messages a day, spread across 9am to 8pm EST. and we had kafka sitting in the middle of it all. a distributed log built to handle trillions of events at linkedin, doing the job that redis was born to do.

a week later, kafka was gone. bill dropped to $180. nothing broke. campaigns actually got faster.
...
in reality, two things were quietly killing us.

## why kafka in the first place

honest answer: it felt like the serious choice.

we were building a system that sends messages at scale — campaigns, sequences, multi-channel delivery. every architecture post you read for something like this says "use an event-driven system." kafka is the name that keeps coming up. it's what the big companies use. it looked like the right foundation to grow into.

there was also a real technical reason. the api and worker are separate services. we needed something in between to handle the handoff — take a launch request, queue the work, let the worker pick it up asynchronously. kafka fit that description. it gave us decoupling, async processing, and the ability to replay events if something went wrong.

what we didnt think hard enough about: kafka solves those problems at a scale we didnt have, with guarantees we didnt need, at a cost that made no sense for where we were. we reached for it because it was familiar from blog posts, not because we had the problems it was built for.
...
that's the kafka model — and its the right model when ordering matters across related events. but our jobs werent related. campaign A has nothing to do with campaign B. there's no reason campaign B should wait for campaign A's 500 contacts to finish before it even starts.
...
you can throw more consumers at this. more consumers means more MSK throughput, which means more brokers, which means a bigger bill. we were already on the smallest possible MSK cluster and it was $1000/month. scaling up wasnt an option — scaling up was the problem.
...
we had a job queue. we just kept calling it an event stream.
...
$10,000/year. the migration took a week. we had ten users so we scheduled an hour of downtime and cut over. the migration was cheaper than one month of MSK.

## "but what about durability"

### durability

Redis with AOF persistence covers durability for job state. BullMQ stores every job — its data, attempts, failures, result. if a worker crashes mid-job, BullMQ marks it stalled and another worker picks it up. we havent lost a job.
...
what we did lose: completed jobs get cleaned up after 24 hours. if you need a permanent audit trail you log to a database — which we already do anyway. kafka wasnt replacing that. it was sitting on top of it.
...
if you outgrow BullMQ, the migration will be a real engineering project — and by then youll have the revenue to pay for it. until then, every "future-proof" choice is a tax you pay every month for a future that hasnt arrived.

we paid $820/month for a year before we fixed it. damm.
