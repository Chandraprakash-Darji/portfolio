...
export default function FramesLightbox({ images }: Props) {
  ...
  const download = useCallback(async () => {
    ...
    if (!img) return;
    (window as any).umami?.track('frame-download', { frame: img.pathname });
    try {
  ...
  useEffect(() => {
    ...
    return () => {
      delete (window as any).__openFramesLightbox;
    };
  }, [openAt]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-4 left-4 z-10 font-mono text-xs text-muted-foreground">
        ...
        {current.alt && (
          <span className="ml-3 italic text-muted-foreground/70">
            {current.alt}
          </span>
        )}
      </div>
